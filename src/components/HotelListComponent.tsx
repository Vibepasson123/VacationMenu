...
import { CardContainer, CardDescription, CardFavoriteContainer, CardFooter, CardImageContainer, FlexRow, MainCardTitle, PhoneNumberContainer, PhoneText, PriceContainer, PriceContainerTag, PriceText, TextsWrapper } from '../styled/styled';
...
interface HotelListComponentProps {
  fav?: boolean;
}
const HotelListComponent: React.FC<HotelListComponentProps> = ({ fav = false }) => {
	const scrollY = React.useRef(new Animated.Value(0)).current;
  const { favorites, hotelList } = useHotelManger();
  const filteredHotelList = fav
  ? hotelList.filter((hotel) => favorites.includes(hotel.id))
  : hotelList;
...
const Card = ({ item, scale, opacity }: { item: any, scale: any, opacity: any, onPress: () => void }) => {
  const navigation = useNavigation() as any;
  const { addToFavorites, favorites } = useHotelManger();
  ...
  return (
    <Animated.View style={{ transform: [{ scale }], opacity }}>
      <CardContainer activeOpacity={0.8} onPress={()=>navigation.navigate('HoltelDetails', { item })}>
        <CardImageContainer>
          <ImageCarousel images={item?.gallery} height={100} width={150} animation autoSlide />
        </CardImageContainer>
        <TextsWrapper>
          <MainCardTitle>
              {item?.location?.city} <Icon name="location-outline" size={14} color="#0EA5E9"  />
          </MainCardTitle>
          <CardDescription numberOfLines={2}>{item?.name.toUpperCase()}</CardDescription>
          <FlexRow spaceBetween>
            <FlexRow>
              <MaterialIcons name="reviews" size={24} color="#16A34A"  />
        ...
        <CardFavoriteContainer activeOpacity={0.8} onPress={()=> addToFavorites(item.id)}>
          <Icon name="bookmark" size={34} color={favorites.includes(item.id) ? '#BE123C' : '#F9A8D4'} />
...
export default  React.memo(HotelListComponent);
